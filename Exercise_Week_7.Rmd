...
31798_at: 3rd most differentially expressed gene in present10 - absent10 comparison (coef. = 1) and the 2nd most in present48 - absent48 comparison (coef. = 2).

```{r}

barplot( exprs(eset)["31798_at",], las=2, cex.names=.7 )  
topTable(fit2,coef=1)[3,]

exprs(eset)["31798_at",]

# locFC (coef 1)
g1mean10 <- mean(exprs(eset)["31798_at",c(3,4)])
g2mean10 <- mean(exprs(eset)["31798_at",c(1,2)])
fc10 <- g1mean10 - g2mean10
fc10==topTable(fit2,coef=1)[3,1]

# locFC (coef 2)
g1mean48 <- mean(exprs(eset)["31798_at",c(7,8)])
g2mean48 <- mean(exprs(eset)["31798_at",c(5,6)])
fc48 <- g1mean48 - g2mean48
topTable(fit2,coef=2)[2,1]
round(fc48,4) == round(topTable(fit2,coef=2)[1,1],4)


# AveExpr
aveExp <- mean(exprs(eset)["31798_at",])
topTable(fit2,coef=1)[3,2]==aveExp
```

*I calculated the logFC for both contrasts (coefficient 1 and 2) and compared it to the respective values in the fit2 table. For the first contrast (10) the manually calculated logFC is not the same as in the fit2 table. For the second comparison (48) it however is the same. The manually calculated aveExp value is also the same as the one obtained from the fit2 table.*
...
alt_design <- as.matrix(rbind(c(1,0,0,0),c(1,0,0,0),c(1,0,1,0),c(1,0,1,0),c(1,1,0,0),c(1,1,0,0),c(1,1,0,1),c(1,1,0,1)))
colnames(alt_design) <- levels(f)
alt_design

alt_fit <- lmFit(eset, alt_design)
alt_fit <- eBayes(alt_fit)
topTable(alt_fit)
topTable(fit2)
```
